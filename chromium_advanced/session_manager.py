...
from chromium_advanced.session_runtime_cleanup import terminate_runtime_processes
from chromium_advanced.session_runtime_view import build_active_session_view, sessions_for_profile_view
...
class SessionManager:
    ...
    def _build_active_session_view_locked(self, occupancy_map: Optional[Dict[str, Dict]] = None) -> List[SessionRecord]:
        ...
        return build_active_session_view(
            live_sessions=list(self._sessions_by_id.values()),
            occupancy_map=occupancy_map,
            session_record_cls=SessionRecord,
            resource_lease_session_cls=ResourceLeaseSession,
        )
    ...
    def _sessions_for_profile_view_locked(
        ...
        if not isinstance(occupancy_map, dict):
            occupancy_map = self.list_profile_occupancy(reconcile=False, tolerate_lock_timeout=True)
        return sessions_for_profile_view(
            profile_name=profile_name,
            live_sessions=list(self._sessions_by_id.values()),
            occupancy_map=occupancy_map,
            session_record_cls=SessionRecord,
