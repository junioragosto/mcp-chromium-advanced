...
4. 它连接的是真实 Chromium Profile。
   项目会用真实的 `user-data-dir` 和 `profile-directory` 启动浏览器，再通过 `undetected_chromedriver` 把 Selenium 挂接上去。
5. 它除了 MCP 控制，还内置了保活工作流。
...
项目会使用真实的 `user-data-dir` 和 `profile-directory` 启动 Chromium，再通过 `undetected_chromedriver` 接入 Selenium。这样 worker 可以复用真实的 cookie、登录态、local storage、扩展和其他持久化浏览器状态。
